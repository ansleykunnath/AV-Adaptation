...
# set laptop volume to 40, speaker volume vertical
...
instructions_1 = visual.TextStim(win=win, name='instructions_1',
    text="Welcome to the experiment!\n\nYou will hear a series of syllables. Your job is to press '1' on the keyboard whenever you are presented with noise.\n\n\n\nTry to answer as accurately as possible. If you're not sure, just make your best guess.\n\n\n(Press 3 to continue.)",
    font='Open Sans',
...
test1 = sound.Sound('practice.wav', secs=11.5)
test1.setVolume(1)
white_cross = visual.ImageStim(
    win=win,
    name='white_cross', 
    image='Fixation2.png', mask=None, anchor='center',
...
practice_correct = visual.TextStim(win=win, name='practice_correct',
    text="\nYou should have just pressed 1 when you heard the noise.\n\n\nRemember to only press 1 when you hear the noise and try to be as accurate as possible!",
    font='Open Sans',
    pos=(0, 0), height=0.05, wrapWidth=None, ori=0.0, 
    color='white', colorSpace='rgb', opacity=None, 
    languageStyle='LTR',
...
rep_50 = sound.Sound('ba_500ms.wav', secs=0.5, name='rep_50')
rep_50.setVolume(1)
rep_50_2 = sound.Sound('ba_500ms.wav', secs=0.5, name='rep_50_2')
rep_50_2.setVolume(1)
...
alt_50 = sound.Sound('ba_500ms.wav', secs=0.5, name="alt_50")
alt_50.setVolume(1)
alt_1000 = sound.Sound('ga_500ms.wav', secs=0.5, name='alt_1000')
alt_1000.setVolume(1)
...
att_50 = sound.Sound('ba_500ms.wav', secs=0.5, name="att_50")
att_50.setVolume(1)
static = sound.Sound('noise_500ms.wav', secs=0.5, name='static')
...
def practice_trials():
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if grace_period.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > grace_period.tStartRefresh + 1-frameTolerance:
                # keep track of stop time/frame for later
    ...
    continueRoutine = True
    routineTimer.add(11.500)
    # update component parameters for each repeat
    test1 = sound.Sound('practice.wav', secs=11.5, name='test1')
    test1.setVolume(1)
    white_cross = visual.ImageStim(
        win=win,
        name='white_cross', 
    ...
    # keep track of which components have finished
    test_trialComponents = [test1, white_cross]
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if test1.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > test1.tStartRefresh + 11.5-frameTolerance:
                # keep track of stop time/frame for later
...
# ------Prepare to start Routine "correct_answer"-------
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if practice_correct.status == NOT_STARTED and tThisFlip >= 1.0-frameTolerance:
            # keep track of start time/frame for later
        ...
        if practice_correct.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > practice_correct.tStartRefresh + 5.0-frameTolerance:
                # keep track of stop time/frame for later
...
def aud_rep():
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if white_cross.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > white_cross.tStartRefresh + ITI-frameTolerance:
                # keep track of stop time/frame for later
                white_cross.tStop = t  # not accounting for scr refresh
                white_cross.frameNStop = frameN  # exact frame index
                win.timeOnFlip(white_cross, 'tStopRefresh')  # time at next scr refresh
...
def aud_alt():
    ...
    routineTimer.add(2.500000)
    # update component parameters for each repeat
    alt_50.setSound('ba_500ms.wav', secs=0.5)
    alt_50.setVolume(1, log=False)
    alt_1000.setSound('ga_500ms.wav', secs=0.5)
    alt_1000.setVolume(1, log=False)
    response_alt.keys = []
    response_alt.rt = []
    _response_alt_allKeys = []
    # keep track of which components have finished
    auditory_alternationComponents = [alt_50, alt_1000, response_alt, white_cross]
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if alt_50.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > alt_50.tStartRefresh + 0.500-frameTolerance:
                # keep track of stop time/frame for later
                ...
                alt_50.stop()
        # start/stop alt_1000
        if alt_1000.status == NOT_STARTED and tThisFlip >= 1.00-frameTolerance:
            # keep track of start time/frame for later
        ...
        if alt_1000.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > alt_1000.tStartRefresh + 0.500-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if response_alt.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > response_alt.tStartRefresh + 2.5-frameTolerance:
                # keep track of stop time/frame for later
...
def aud_att():
    ...
    routineTimer.add(2.500000)
    # update component parameters for each repeat
    att_50.setSound('ba_500ms.wav', secs=0.5)
    att_50.setVolume(1, log=False)
    static.setSound('noise_500ms.wav', secs=0.5)
    static.setVolume(0.1, log=False)
    response_att.keys = []
    response_att.rt = []
    _response_att_allKeys = []
    # keep track of which components have finished
    auditory_attentionComponents = [att_50, static, response_att, white_cross]
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if att_50.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > att_50.tStartRefresh + 0.50-frameTolerance:
                # keep track of stop time/frame for later
                ...
                att_50.stop()
        # start/stop static
        if static.status == NOT_STARTED and tThisFlip >= 1.0-frameTolerance:
            # keep track of start time/frame for later
        ...
        if static.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > static.tStartRefresh + 0.5-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if response_att.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > response_att.tStartRefresh + 2.5-frameTolerance:
                # keep track of stop time/frame for later
